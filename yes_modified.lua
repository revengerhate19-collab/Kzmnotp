...
return function(ctx)
    ...
    local otpconn = nil;
    local otpCamConn = nil;
    local otpCamCF = nil;
    local otpCamType = nil;
    local otpCamSubject = nil;

    function ms.orbitTP(v)
        if not v then
            if otpconn then otpconn:Disconnect(); otpconn = nil; end;
            if otpCamConn then otpCamConn:Disconnect(); otpCamConn = nil; end;
            local cam = workspace.CurrentCamera;
            if cam and otpCamCF then
                cam.CFrame = otpCamCF;
                if otpCamType then cam.CameraType = otpCamType; end;
                if otpCamSubject then cam.CameraSubject = otpCamSubject; end;
            end;
            otpCamCF, otpCamType, otpCamSubject = nil, nil, nil;
            return;
        end;
        if otpconn then return; end;

        -- Freeze the camera in world space while the character is being TP'd.
        local cam = workspace.CurrentCamera;
        if cam then
            otpCamCF = cam.CFrame;
            otpCamType = cam.CameraType;
            otpCamSubject = cam.CameraSubject;
            cam.CameraType = Enum.CameraType.Scriptable;
            cam.CFrame = otpCamCF;
            otpCamConn = s.runs.RenderStepped:Connect(function()
                local c = workspace.CurrentCamera;
                if c and otpCamCF then
        ...
        otpconn = s.runs.Heartbeat:Connect(function(dt)
            ...
            -- Randomize the angle, move a little closer, and face the enemy.
            lastAngle = math.random() * math.pi * 2;
            local r  = num("OrbitRange", 6);
            local tp = thrp.Position;
            local nx = tp.X + math.cos(lastAngle) * r;
            local nz = tp.Z + math.sin(lastAngle) * r;
            myhrp.AssemblyLinearVelocity = Vector3.zero;
            myhrp.CFrame = CFrame.new(Vector3.new(nx, tp.Y, nz), tp);
            if otpCamCF then
                local c = workspace.CurrentCamera;
                if c then
                    c.CameraType = Enum.CameraType.Scriptable;
